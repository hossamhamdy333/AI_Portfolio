...
Raw SST-5 (HuggingFace)
    ...
    → ONNX model pushed to the Hugging Face Hub
...
EDA turned up a few things that shaped the cleaning step: 10 duplicate sentences in train, 1.6% of training sentences at 3 words or fewer (mostly still meaningful, e.g. *"too bad."*, *"stay away."*), and a vocabulary of roughly 16.4K unique tokens where about half the words appear only once. That long tail is part of why classical bag-of-words models struggle here and why pretrained embeddings or contextual representations are worth trying. Classes are moderately imbalanced (negative and positive are the two largest, neutral and very-negative the smallest), which is why class weighting shows up in both the BiLSTM and BERT training runs.
...
Combined word-level (1–2 grams) and character-level (3–5 grams) TF-IDF via `FeatureUnion`, so the model picks up both sentiment phrases ("not good") and morphological/OOV patterns that word n-grams alone would miss. `C` was tuned with 5-fold `GridSearchCV` over `[0.01, 0.1, 1.0, 10.0, 100.0]`; `C=1.0` won with CV F1 of 0.400. `class_weight="balanced"` offsets the class imbalance.

**2. BiLSTM** (`notebooks/03_BiLSTM.ipynb`)
2-layer bidirectional LSTM (hidden dim 256, dropout 0.3) on top of 100-dim GloVe embeddings. 92.2% of the vocab (15,279/16,572 tokens) had a pretrained vector; the rest were initialized randomly. About 4M trainable parameters. Trained for up to 10 epochs with a weighted cross-entropy loss (class weights ranging 0.73–1.57) and gradient clipping. Best checkpoint was epoch 7 (val F1 0.4345), saved via early best-model checkpointing rather than just taking the last epoch.

**3. BERT fine-tuning** (`notebooks/04_BERT-Fine-tuning.ipynb`)
`bert-base-uncased` (109.5M params), fully fine-tuned rather than frozen or probed, for 5 epochs, batch size 32, LR 2e-5 with warmup, weight decay 0.01, mixed precision (fp16), and a custom `WeightedTrainer` subclass to apply the same class weighting during loss computation. Tracked with Weights & Biases.

All three log metrics to MLflow, so the run history and comparison numbers stay queryable later instead of just being printed once and forgotten.
...
BERT wins on accuracy by a wide margin, about 10 F1 points over the other two, with a meaningfully higher AUC-ROC (0.836 vs roughly 0.75). What stands out more is how close TF-IDF and the BiLSTM land. The BiLSTM barely edges out classical ML despite being the "deeper" model, which says something about how hard 5-way sentiment actually is on short, single-sentence reviews: there isn't much sequential structure left for an LSTM to exploit that a strong TF-IDF representation hasn't already captured.

Per-class, "very positive" is the strongest category for every model (F1 in the 0.54–0.65 range), and "neutral" is the weakest across the board (0.28–0.37 F1). Reviews that read as lukewarm or mixed are genuinely ambiguous even to a human labeler, so this looks more like a property of the task than a modeling bug. Confusion is almost entirely between adjacent classes (negative/very negative, positive/very positive), rarely between opposite ends of the scale. See the confusion matrices in `outputs/figures/`.
...
**Latency and size** were also benchmarked directly (`outputs/models/`, timed on the same hardware). TF-IDF and the BiLSTM are both sub-millisecond per sample and small enough to fit almost anywhere. BERT is roughly 5x slower per inference and 30–150x larger on disk. If latency or edge deployment mattered more than raw accuracy, TF-IDF or the BiLSTM would be the right call. Since accuracy was the priority here, BERT is the one hosted publicly for inference.
...
The trained BERT model is hosted on the Hugging Face Hub at [`hossam3759180/sentiment_forge`](https://huggingface.co/hossam3759180/sentiment_forge): ONNX weights plus tokenizer, no server required. Open `notebooks/05_comparison.ipynb` and run the "Try it yourself" cell, which pulls the model straight from the Hub:

```python
import onnxruntime as ort
from huggingface_hub import snapshot_download

local_dir = snapshot_download(repo_id="hossam3759180/sentiment_forge")
session = ort.InferenceSession(f"{local_dir}/model.onnx")

# tokenize + run inference — see the notebook for the full predict() function
```

```
Text       : this movie was absolutely wonderful and touching
Prediction : very positive (confidence: 0.955)

Text       : terrible film , complete waste of time
Prediction : very negative (confidence: 0.918)

Text       : it was okay , nothing special
Prediction : neutral (confidence: 0.632)
```
...
To reproduce a training run, open any of the numbered notebooks. Each one loads its config from `configs/config.yaml`, pulls data via DVC, and logs to MLflow/W&B.

## The MLOps pieces, not just the models

DVC handles versioning for datasets and trained model binaries (`.pkl`, `.pt`, ONNX exports) without bloating git; only the `.dvc` pointer files are actually committed. MLflow logs every model's metrics per run, so results stay comparable across experiments instead of living only in terminal output. Weights & Biases tracks training curves (loss, val F1 per epoch) for the BiLSTM and BERT runs.

The BERT model was exported to ONNX and its output was numerically checked against the original PyTorch model before going anywhere else (max absolute difference: 0.0008). The ONNX model and tokenizer are hosted publicly on the Hugging Face Hub and loaded on demand via `snapshot_download`, so anyone can run inference without retraining or hosting anything themselves.

Testing is 7 unit tests via pytest, with GitHub Actions running them automatically on every push to this folder. Every hyperparameter (TF-IDF settings, LSTM dims, BERT training args, model repo reference) lives in one `config.yaml` rather than being scattered across notebooks.
