...
Feature engineering happens in Python (`src/features.py`), with the same module imported by both the training notebook and the (in-progress) serving API, so there's one definition of each feature rather than two that can drift apart. Every feature is built to only use information available at or before the day it's predicting for — lags, rolling means and standard deviations (computed with a one-day shift before rolling, so the window never includes the current day), days-since-last-sale, and calendar features like day of week and distance to the nearest Christmas.
